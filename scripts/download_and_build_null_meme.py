#!/usr/bin/env python3
"""
Download & Build Full Null Simulation Dataset from Silverback
------------------------------------------------------------
1. Syncs all completed MEME JSON results, simulated NEXUS alignments, and tree files
   from silverback.temple.edu (~/Projects/TOGA_MEME_NULL_SIMS/) to local null_simulations/.
2. Parses site-by-site Maximum Likelihood MEME estimates (LRT, alpha, beta_neg, p_neg, beta_pos, p_pos, p_val).
3. Precomputes NPZ features (distance matrix, 4D MDS coordinates, variable sites) in msa_cache_npz/.
4. Populates SQLite database (meme_results.db) with exact MEME ML inference targets.
"""
...
def rsync_null_simulations_from_silverback(local_null_dir):
    """Rsync all null simulation alignments, trees, and MEME JSON results from silverback."""
    os.makedirs(local_null_dir, exist_ok=True)
    print(" -> Syncing MEME JSON results from silverback...")
    cmd_results = f"rsync -avz --include='*.MEME.json' --exclude='*' silverback.temple.edu:~/Projects/TOGA_MEME_NULL_SIMS/results/ {local_null_dir}/"
    subprocess.run(cmd_results, shell=True, check=True)
    
    print(" -> Syncing simulated NEXUS alignments from silverback...")
    cmd_sims = f"rsync -avz --include='*.replicate.1.nex' --exclude='*' silverback.temple.edu:~/Projects/TOGA_MEME_NULL_SIMS/sims/ {local_null_dir}/"
    subprocess.run(cmd_sims, shell=True, check=True)
    
    print(" -> Syncing Newick tree files from silverback...")
    cmd_trees = f"rsync -avz --include='*_tree.nwk' --exclude='*' silverback.temple.edu:~/Projects/TOGA_MEME_NULL_SIMS/trees/ {local_null_dir}/"
    subprocess.run(cmd_trees, shell=True, check=True)
...
def parse_meme_json_targets(json_path):
    ...
    try:
        if os.path.getsize(json_path) < 100:
            return None
...
def main():
    ...
    print("=" * 80)
    print("🚀 SYNCING & BUILDING FULL SILVERBACK MEME NULL SIMULATION DATASET")
    print("=" * 80)
    
    # 1. Sync files from silverback
    try:
        rsync_null_simulations_from_silverback(local_null_dir)
    except Exception as e:
        print(f"[!] Sync warning: {e}")
        
    # 2. Match completed MEME JSONs with NEXUS alignments and trees
    ...
    for json_f in json_files:
        ...
        gene_id = base.replace('null_', '')
        tree_f = os.path.join(local_null_dir, f"{gene_id}_tree.nwk")
    ...
    print(f" -> Processing {len(tasks)} completed simulation alignments in parallel...")
    ...
    print(f"\n -> Successfully processed {genes_added} null simulation genes ({total_null_sites:,} MEME-fitted sites).")
    
    if all_site_records:
        print(f" -> Inserting {len(all_site_records):,} MEME ML-inferred site targets into SQLite database '{db_path}'...")
